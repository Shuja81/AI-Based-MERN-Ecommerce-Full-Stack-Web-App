...
router.post("/add", auth, role.check(ROLES.Admin), async (req, res) => {
    try {
        ...
        if (!name || !email) {
            return res
                .status(400)
                .json({ error: "You must enter name and email." });
        }

        if (!password) {
            return res
                .status(400)
                .json({ error: "You must set a password for the merchant." });
        }
        ...
        res.status(200).json({
            success: true,
            message: `Merchant account created successfully. They can now login with email: ${email}`,
